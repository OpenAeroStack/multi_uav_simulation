...
say "════════════════════════════════════════════════════════════"
say ""
say "  Initialisation is complete and verified. Next, in NEW terminals:"
say ""
say "      2)  ./scripts/netns/detector_start.sh    # Pi detectors + receivers"
say "      3)  ./scripts/netns/run_missions.sh      # fly"
say ""
say "  Start the detectors FIRST: they take ~15 s to load OpenVINO, and a"
say "  mission flown before they are up produces a flight with no detections."
say "  Step 3 re-checks every stage before arming, so a half-built stack fails"
