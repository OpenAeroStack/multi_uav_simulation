...
Common paths:
```bash
export ARDUPILOT_HOME="$HOME/wimukthi/ardupilot"                   # installed in home directory
...
To quickly validate that NS-3 TapBridge forwarding is working with a simple `ping` between UAV namespaces, you can instead use a shared subnet:
...
4. Open the NetAnim XML (generated by NS-3 `--animFile`, usually `/tmp/three_uav_anim.xml` when using the launch script) in the NetAnim GUI.
