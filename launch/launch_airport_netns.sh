...
# 4) Launch Gazebo Airport
# Use airport_world(regular iris models with fdm_addr=127.0.0.1)
# because SITL runs in root namespace alongside Gazebo.
echo "=== [4/8] Launching Gazebo Airport (background) ==="
...
for i in 1 2 3; do
  ...
  # 2. Gazebo Control Tunnel (TCP) - Host 9002 -> Namespace 10.42.0.11:9002
  socat TCP-LISTEN:${GAZEBO_PORT},fork,reuseaddr TCP:${NS_IP}:${GAZEBO_PORT} &

  # 3. Gazebo Telemetry Tunnel (UDP) - Host 9003 -> Namespace 10.42.0.11:9003
  socat UDP-LISTEN:${SITL_IN_PORT},fork,reuseaddr UDP-SENDTO:${NS_IP}:${SITL_IN_PORT} &
  
  # 4. SITL Return Tunnel (UDP) - Namespace 10.42.0.11:9003 -> Host 127.0.0.1:9003
  # This is the most critical: The drone needs to send data BACK to Gazebo
  sudo ip netns exec "${NS}" socat UDP-LISTEN:${SITL_IN_PORT},fork,reuseaddr UDP-SENDTO:127.0.0.1:${SITL_IN_PORT} &
...
# CHANGE: Comment out the trap temporarily to stop the auto-kill
# trap cleanup_ns3 EXIT INT TERM 
