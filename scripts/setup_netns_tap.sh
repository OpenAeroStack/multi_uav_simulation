...
for i in $(seq 1 "$UAV_COUNT"); do
  ...
  # Add a default route inside the namespace so UDP (AP_DDS/micro-ROS) traffic
  # can reach the gateway/host endpoint on 10.42.0.1.
  ip netns exec "$ns_name" ip route replace default via 10.42.0.1 dev eth0

  # Allow AP_DDS UDP traffic on the per-UAV port (2018 + i) so ports 2019-2021
  # are not blocked by namespace-local firewall rules.
  ap_dds_port=$((2018 + i))
  ip netns exec "$ns_name" iptables -C INPUT  -p udp --dport "$ap_dds_port" -j ACCEPT 2>/dev/null || \
    ip netns exec "$ns_name" iptables -A INPUT  -p udp --dport "$ap_dds_port" -j ACCEPT
  ip netns exec "$ns_name" iptables -C OUTPUT -p udp --dport "$ap_dds_port" -j ACCEPT 2>/dev/null || \
    ip netns exec "$ns_name" iptables -A OUTPUT -p udp --dport "$ap_dds_port" -j ACCEPT
