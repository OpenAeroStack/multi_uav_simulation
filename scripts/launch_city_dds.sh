...
# BEGIN GAZEBO GPU CONFIGURATION
USE_NVIDIA_GPU="${USE_NVIDIA_GPU:-1}"

configure_gazebo_gpu() {
    if [[ "$USE_NVIDIA_GPU" != "1" ]]; then
        echo "NVIDIA GPU offload disabled by configuration."
        return 0
    fi

    if ! command -v nvidia-smi >/dev/null 2>&1; then
        echo "NVIDIA GPU not detected; using the system default OpenGL renderer."
        return 0
    fi

    if ! nvidia-smi >/dev/null 2>&1; then
        echo "WARNING: NVIDIA driver is installed but the GPU is unavailable." >&2
        echo "Gazebo will use the default OpenGL renderer." >&2
        return 0
    fi
    ...
    echo "NVIDIA PRIME render offload enabled for Gazebo."
}
# END GAZEBO GPU CONFIGURATION
...
# BEGIN SINGLE GAZEBO GUI FUNCTION
start_gazebo_gui_once() {
    if [[ "${ENABLE_GAZEBO_GUI:-1}" != "1" ]]; then
        echo "Gazebo GUI disabled; gzserver remains headless."
        return 0
    fi
    ...
    # Remove any untracked viewer left from an earlier run.
    pkill -TERM -x gzclient 2>/dev/null || true
    sleep 1

    GZCLIENT_LOG="/tmp/gzclient_city.log"
    : >"$GZCLIENT_LOG"

    gzclient >"$GZCLIENT_LOG" 2>&1 &
    GZCLIENT_PID=$!
    ...
    # Give the Gazebo marker service time to connect.
    sleep 3

    echo "Gazebo GUI started once:"
    echo "  PID=$GZCLIENT_PID"
    echo "  log=$GZCLIENT_LOG"
}
# END SINGLE GAZEBO GUI FUNCTION
