...
int main(int argc, char* argv[])
{
  ...
  for (uint32_t i = 0; i < 3; ++i)
    {
      TapBridgeHelper tap;
      // Force UseLocal mode for TapBridge on WiFi because WifiNetDevice does not
      // support SendFrom(), which UseBridge relies on.
  ...
  if (enableFlowMonitor && monitor)
    {
      ...
      // Print per-UAV AP_DDS flow stats (destination UDP ports 2019-2021).
          ...
          for (const auto& entry : stats)
            {
              ...
              if (t.destinationPort >= 2019 && t.destinationPort <= 2021)
                {
                  const uint32_t uavIndex = static_cast<uint32_t>(t.destinationPort - 2018);
                  const double avgDelaySec = (s.rxPackets > 0) ? (s.delaySum.GetSeconds() / static_cast<double>(s.rxPackets)) : 0.0;
                  NS_LOG_UNCOND("AP_DDS UAV" << uavIndex
