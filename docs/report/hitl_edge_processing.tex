...
\begin{table}[htbp]
\centering
\caption{Measured characteristics of the two links. Sensor path over sixty
ICMP samples, radio path over thirty; zero packet loss on both.}
...
wired path on average, and its variation is larger by more than two orders of
magnitude.
...
\(77.4\)\,ms against the \(74.5\)\,ms reported here --- agreement within
\(4\%\). The maximum differed considerably more, \(242.6\)\,ms against
\(158.0\)\,ms, which is expected of a single worst-case packet and is the
reason the mean and jitter are the figures relied upon.

The distribution within the radio path is itself informative. Its minimum of
\(10.1\)\,ms against a mean of \(74.5\)\,ms is a spread of some \(7\times\) on
...
At \(5\)\,Hz the wired path carries \(110\)\,Mbps, approximately \(11\%\) of
the \(1000\)\,Mb/s negotiated link. Frames are delivered at \(5.000\)\,Hz with
an inter-arrival standard deviation of \(4.5\)\,ms, matching the source rate
exactly, with zero dropped packets. An earlier configuration at \(20\)\,Hz
carried \(481\)\,Mbps at \(42{,}544\) packets per second, also without loss.

\paragraph{The shared cable does not distort the measurements.}
Both logical links share one physical cable, separated only by VLAN tags, so
it is reasonable to ask whether the sensor stream delays the traffic measured
alongside it. It does not. The sensor-path figures in
Table~\ref{tab:hitl-links} were taken \emph{while} the camera was streaming;
an idle-cable measurement over the same number of samples gives a mean of
\(1.4\)\,ms, unchanged. Jitter under load is in fact lower than at idle, a
continuously active interface avoiding the wake-up latency of idle power
states.

Two mechanisms account for this. The two flows travel in opposite directions
on a full-duplex link, and the sensor stream occupies only \(11\%\) of
capacity, so no standing queue can form. Queue management was verified rather
than assumed: both machines run \texttt{fq\_codel} with a \(5\)\,ms target,
and byte queue limits on the edge node had reduced its driver ring to
approximately one packet.
...
\(1{,}093\)\,ms within the live pipeline, averaged over \(480\) consecutive
frames of a patrol flight: a difference of approximately \(470\)\,ms, or some
\(43\%\) of the time the detector spends on each frame.

The edge node receives frames at \(5\)\,Hz while completing inference at
approximately \(0.9\)\,Hz, discarding roughly \(80\%\) of what it receives.
Discarded frames are not free: each requires reassembly of some \(1{,}900\) UDP
...
The mechanism is processor contention rather than queueing delay. Two
observations separate them. Inference is timed around the model invocation
alone, excluding message reception and format conversion, so the additional
time is spent inside the model call rather than waiting ahead of it. And the
network itself is demonstrably not congested: latency on the sensor link is
unchanged between an idle cable and one carrying the full \(110\)\,Mbps
stream, as reported in Section~\ref{sec:hitl-results}.

Reducing the camera rate at source from \(20\)\,Hz to \(5\)\,Hz --- still
several times the rate the detector can consume --- reduced link occupancy from
\(481\)\,Mbps to \(110\)\,Mbps and inference latency correspondingly. The
...
An earlier configuration in which the edge node also published a
\(2.76\)\,MB annotated image on every detection measured \(1{,}343\)\,ms. That
return stream was removed: it doubled the traffic on a link intended to carry
sensor data in one direction only, and it is not part of the architecture under
test. The figures above are for the delivered configuration, in which the edge
node emits detection results and nothing else.
