...
for tap in "${TAPS[@]}"; do
    ...
    flags="$(ip -o link show dev "$tap" | sed -n 's/.*<\([^>]*\)>.*/\1/p')"
...
bash -lc '
    ...
    source "$1"
    exec gazebo --verbose "$2" \
        -s libgazebo_ros_init.so \
        -s libgazebo_ros_factory.so
' gazebo-shell "$PROJECT_DIR/setup.sh" "$PROJECT_DIR/worlds/city_3uav.world" \
    >"$GAZEBO_LOG" 2>&1 &
GAZEBO_PID=$!
...
launch_sitl() {
    ...
    sudo ip netns exec "$namespace" \
        ...
        "$work_dir" "$BINARY" \
        --model gazebo-iris --speedup 1 --sysid "$sysid" \
        --instance "$instance" --defaults "$defaults" \
        --sim-address "$gazebo_ip" --home "$HOME_GPS" \
        --serial0="tcp:0.0.0.0:$mavlink_port" \
        --serial1="udpclient:10.42.0.10:$dds_port" \
        >"$log_file" 2>&1 &
    SITL_PIDS+=("$!")
...
launch_sitl uav1 0 1 5760 2019 172.31.1.1 "$UAV1_DEFAULTS"
launch_sitl uav2 1 2 5770 2020 172.31.2.1 "$UAV2_DEFAULTS"
launch_sitl uav3 2 3 5780 2021 172.31.3.1 "$UAV3_DEFAULTS"
