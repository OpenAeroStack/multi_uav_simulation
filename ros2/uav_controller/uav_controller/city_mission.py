...
# Barriers synchronise all 3 threads at key phase transitions
barrier_takeoff   = threading.Barrier(N_DRONES)
barrier_positions = threading.Barrier(N_DRONES)
barrier_sweep     = threading.Barrier(N_DRONES)
...
class CityMission(Node):
    ...
    def _publish_cluster_status(self):
        s = self.states
        if any(s[i].lat is None for i in [1,2,3]):
            return
        msg = String()
        msg.data = (
            f'CLUSTER_STATUS | '
            f'HEAD=UAV1(alt={s[1].rel_alt:.1f}m mode={s[1].mode}) | '
            f'M1=UAV2(lat={s[2].lat:.5f} lon={s[2].lon:.5f} '
            f'alt={s[2].rel_alt:.1f}m) | '
            f'M2=UAV3(lat={s[3].lat:.5f} lon={s[3].lon:.5f} '
            f'alt={s[3].rel_alt:.1f}m)'
        )
